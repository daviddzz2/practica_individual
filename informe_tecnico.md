...
Además, el sistema incorpora un **mecanismo de prioridades** para atender primero a usuarios con necesidades especiales (como personas mayores, discapacitados o emergencias), asegurando una distribución equitativa y eficiente de los recursos.

## 3. Sistema de Prioridades

Para simular escenarios reales donde ciertos usuarios requieren atención prioritaria, el sistema implementa un esquema de prioridades binario:

- **Prioridad 0 (Normal)**: Usuarios estándar sin requerimientos especiales.
- **Prioridad 1 (Alta)**: Usuarios con necesidades especiales que deben ser atendidos con preferencia.

### Generación de Prioridades

- **Automática**: El 20% de las solicitudes generadas aleatoriamente tienen prioridad alta, simulando la proporción típica de usuarios con necesidades especiales en un edificio.
- **Manual**: Los usuarios pueden especificar la prioridad al introducir solicitudes desde la interfaz interactiva.

### Lógica de Atención Prioritaria

Cuando un ascensor llega a una planta, ordena la cola de espera por prioridad descendente antes de embarcar pasajeros. Esto garantiza que:

1. Los usuarios con prioridad alta suban primero, incluso si llegaron después.
2. Se maximice la eficiencia sin comprometer la equidad.
3. El sistema mantenga su rendimiento general mientras atiende necesidades especiales.

Este enfoque asegura que el tiempo de espera de usuarios prioritarios se reduzca significativamente, mientras que los usuarios normales experimentan un impacto mínimo en sus tiempos de espera promedio.
...
- **`ascensor.py`**: Contiene la clase `Ascensor` (hereda de `threading.Thread`). Maneja la lógica física y algorítmica individual (moverse, abrir puertas, embarcar pasajeros con ordenación por prioridad).
...
- **`persona.py`**: Un hilo generador que actúa como "productor" creando peticiones aleatorias con asignación automática de prioridades (20% alta prioridad).
...
- `[origen] [destino] [prioridad]`: (Ej: `2 9 0`) Introduce manualmente una persona que quiere viajar de la planta 2 a la 9 con prioridad 0 (normal) o 1 (alta).
...
_(Se observa al Ascensor 1 bajando con 1 pasajero y al Ascensor 2 subiendo completamente cargado)._

### Trazabilidad de Eventos (`simulacion.log`)

```text
2026-05-07 22:54:00,268 [INFO] Persona solicita manual: 4 -> 5 (prioridad: 1)
...
2026-05-07 22:54:02,769 [INFO] Ascensor 0: pasajero sube en 4 con destino 5 (prioridad: 1)
