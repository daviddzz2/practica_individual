...
class Solicitud:
    def __init__(self, origen, destino, prioridad=0):
        self.origen = origen
...
class GeneradorPersonas(threading.Thread):
    def run(self):
        while True:
            ...
            if origen != destino:
                prioridad = 1 if random.random() < 0.2 else 0  # 20% de personas con prioridad
                s = Solicitud(origen, destino, prioridad)
                ...
                registrar_solicitud()
                logging.info(f"Persona solicita automático: {origen} -> {destino} (prioridad: {prioridad})")
                if abs(origen - destino) == 1:
                    logging.info(f"🤖 ¡ERES UN VAGO, USA LAS ESCALERAS! (automático de {origen} a {destino})")
            time.sleep(random.uniform(0.5, 3))

def generar_solicitud_manual(origen, destino, prioridad=0):
    s = Solicitud(origen, destino, prioridad)
    try:
        with lock_estado:
            personas_esperando[origen][s.direccion].append(s)
    ...
    cola_solicitudes.put(s)
    registrar_solicitud()
    logging.info(f"Persona solicita manual: {origen} -> {destino} (prioridad: {prioridad})")
    if abs(origen - destino) == 1:
